...
const {ManagerWS, collaborates} = require('../src/model/managerWS');
...
describe('Test Manager',() => {
    ...
    test('Test operation login support ',async ()=>{
        ...
        manager.handler().getOperation("login").do(collaborates(mockServer,payload,manager));
    ...
    test('Test operation login after that run operation toUser support ',async ()=>{
      ...
      socketClientA.send(JSON.stringify({
          'type': "login",
          'name': c1.getUsername(),
          'visible':1,
      }));
      ...
      socketClientB.send(JSON.stringify({
        'type': "login",
        'name': c2.getUsername(),
        'visible':0,
     }));
      ...
      manager.handler().getOperation("login").do(collaborates(mockServer,payloadA,manager));

      expect(message1.type).toEqual("login");
      expect(message1.success).toEqual(true);
      expect(message1.username).toEqual(c1.getUsername());

      manager.handler().getOperation("login").do(collaborates(mockServer,payloadB,manager));
      ...
      manager.handler().getOperation("touser").do(collaborates(mockServer,payloadToUser,manager));
    ...
    test('Test operation login after that run operation listuser support ',async ()=>{
      ...
      manager.handler().getOperation("listUsers").do(collaborates(mockServer,payloadToUser,manager));
